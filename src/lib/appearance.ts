...
const mediaUrl = z.union([z.literal(""), z.url().refine((value) => ["http:", "https:"].includes(new URL(value).protocol))]);
...
export const appearanceSchema = z.object({
  background: z.object({
    ...
    gradient: z.object({
      ...
      stops: z.array(z.object({ color: hexColor, position: z.number().int().min(0).max(100) })).min(2).max(5),
    }),
    mediaUrl,
  ...
  typography: z.object({
    headingFont: z.enum(["Fraunces", "Manrope", "Space Grotesk", "Playfair Display", "DM Serif Display", "Bebas Neue"]),
    bodyFont: z.enum(["Manrope", "Fraunces", "Inter", "Montserrat", "Lora", "Roboto Mono"]),
...
export const DEFAULT_APPEARANCE: AppearanceSettings = {
  background: {
    mode: "gradient",
    solidColor: "#F5F0DE",
    gradient: { type: "linear", angle: 145, stops: [{ color: "#F5F0DE", position: 0 }, { color: "#F8C95C", position: 100 }] },
    mediaUrl: "",
    overlayColor: "#17211B",
    overlayOpacity: 18,
    preset: "sunrise",
  },
  buttons: { shape: "rounded", radius: 18, fill: "shadow", color: "#17211B", textColor: "#FFFFFF", borderColor: "#17211B", shadowColor: "#F06432", height: 58, spacing: 12, hover: "lift", press: "compress" },
  typography: { headingFont: "Fraunces", bodyFont: "Manrope", headingSize: 30, bodySize: 15, weight: 700, letterSpacing: 0, color: "#17211B" },
  layout: { avatarShape: "circle", avatarSize: 96, avatarBorderWidth: 3, avatarBorderColor: "#FFFFFF", bioPlacement: "belowName", alignment: "center", density: "comfortable", contentWidth: 620, socialPlacement: "belowBio" },
  effects: { cursor: "default", cursorColor: "#F06432", trail: "none", clickRipple: false, entrance: "fade", staggerMs: 70 },
  advanced: { removeBranding: false, customCssEnabled: false, detailedAnalytics: false },
};
