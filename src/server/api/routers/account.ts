  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx }) => {
      ...
      if (subscription && !subscription.cancelAtPeriodEnd) {
        try { await getPaymentProvider(subscription.provider).cancelSubscription(subscription); }
        catch { throw new TRPCError({ code: "BAD_GATEWAY", message: "Abonelik sağlayıcıda iptal edilemediği için hesabın silinmedi. Tekrar deneyin." }); }
      }
