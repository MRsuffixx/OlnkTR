...
export const customizationRouter = createTRPCRouter({
  ...
  addDomain: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        return await ctx.db.customDomain.create({
          data: {
        ...
        throw new TRPCError({
          code: "CONFLICT",
          message: "Bu alan adı zaten kullanılıyor.",
        });
      }
    }),
  verifyDomain: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      let verified = false;
      try {
        const records = await resolveTxt(`_olnk.${domain.domainNormalized}`);
        verified = records.some(
          (parts) =>
            parts.join("") === `olnk-verification=${domain.verificationToken}`,
        );
      } catch {
        verified = false;
      }
      ...
      if (!verified)
        throw new TRPCError({
          ...
          message:
            "Doğrulama kaydı henüz görünmüyor. DNS yayılımından sonra tekrar dene.",
        });
      return updated;
    }),
  ...
  uploadStatus: protectedProcedure.query(() => ({
    available: Boolean(storageConfig()),
  })),
  createUpload: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const candidateExtension = input.fileName
        .split(".")
        .at(-1)
        ?.replace(/[^a-z0-9]/gi, "")
        .toLowerCase();
      ...
      await ctx.db.uploadedAsset.create({
        data: {
          userId: ctx.session.user.id,
          objectKey,
          publicUrl,
