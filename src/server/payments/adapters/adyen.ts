...
function config() {
  if (!env.ADYEN_API_KEY || !env.ADYEN_MERCHANT_ACCOUNT || !env.ADYEN_HMAC_KEY || !env.NEXT_PUBLIC_ADYEN_CLIENT_KEY) throw new PaymentConfigurationError("Adyen yapılandırılmamış.");
  return { apiKey: env.ADYEN_API_KEY, merchantAccount: env.ADYEN_MERCHANT_ACCOUNT, hmacKey: env.ADYEN_HMAC_KEY };
}
...
async function adyenPost(url: string, body: Record<string, unknown>, idempotencyKey?: string) {
  const { apiKey } = config();
  const response = await fetch(url, {
    method: "POST",
    headers: { "content-type": "application/json", "x-api-key": apiKey, ...(idempotencyKey ? { "idempotency-key": idempotencyKey } : {}) },
    body: JSON.stringify(body),
    signal: AbortSignal.timeout(15_000),
  });
  const result = await response.json() as Record<string, unknown>;
  if (!response.ok) throw new Error(stringValue(result.message) ?? `Adyen ağ hatası (${response.status}).`);
  return result;
}
...
export const adyenAdapter: PaymentProviderAdapter = {
  ...
  async createCheckoutSession(input: CheckoutInput) {
    ...
    const result = await adyenPost(`${env.ADYEN_API_URL}/sessions`, {
    ...
    if (!sessionId || !sessionData) throw new Error("Adyen geçerli bir oturum döndürmedi.");
    return { kind: "adyen" as const, sessionId, sessionData, externalSessionId: sessionId };
  },
  ...
  async handleWebhook(rawBody: Buffer, headers: Headers) {
    ...
    if (headerSignature) {
      if (!validator.validateHMACSignature(hmacKey, headerSignature, rawBody.toString("utf8"))) throw new WebhookVerificationError("Adyen webhook imzası geçersiz.");
    }
    ...
    return items.flatMap<NormalizedBillingEvent>((item) => {
      ...
      if (code === "CANCEL_OR_REFUND" || code === "REFUNDED_REVERSED") return [{ ...base, type: "refunded", status: "REFUNDED" }];
      if (code === "CHARGEBACK") return [{ ...base, type: "disputed", status: "UNPAID" }];
      return [];
    });
  },
  ...
  async cancelSubscription(subscription) {
    ...
    await adyenPost(`${env.ADYEN_RECURRING_URL}/disable`, { merchantAccount: config().merchantAccount, shopperReference: subscription.userId, recurringDetailReference: subscription.providerPaymentMethodId });
  },
  ...
  async getSubscriptionStatus(subscription) {
    ...
    return { status: expired ? "EXPIRED" : subscription.status, currentPeriodEnd: subscription.currentPeriodEnd ?? undefined, cancelAtPeriodEnd: subscription.cancelAtPeriodEnd };
  },
};

export async function createAdyenRenewal(input: { intentId: string; userId: string; paymentMethodId: string; amountMinor: number; currency: string }) {
  const { merchantAccount } = config();
  return adyenPost(`${env.ADYEN_API_URL}/payments`, {
    amount: { currency: input.currency, value: input.amountMinor },
    reference: input.intentId,
    merchantAccount,
