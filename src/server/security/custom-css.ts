...
const FORBIDDEN_DECLARATION =
  /(url\s*\(|expression\s*\(|javascript\s*:|data\s*:|behavior\s*:|-moz-binding|position\s*:\s*(?:fixed|sticky))/i;
...
export function sanitizeCustomCss(input: string) {
  ...
  root.walkAtRules((rule: AtRule) => {
    ...
    if (
      ["import", "font-face", "namespace", "document", "charset"].includes(name)
    )
      rule.remove();
  });
  root.walkDecls((declaration) => {
    ...
    if (
      FORBIDDEN_DECLARATION.test(candidate) ||
