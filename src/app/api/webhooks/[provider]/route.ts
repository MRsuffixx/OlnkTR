...
const PROVIDERS: Record<string, BillingProvider> = { stripe: "STRIPE", iyzico: "IYZICO", paytr: "PAYTR", adyen: "ADYEN" };
...
export async function POST(request: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  try {
    ...
    return Response.json({ error: invalid ? "Invalid signature" : "Webhook processing failed" }, { status: invalid ? 401 : 500 });
  }
}
