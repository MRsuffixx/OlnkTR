...
export async function POST(
  ...
  if (
    !link?.passwordHash ||
    !hasProAccess(link.user.subscription) ||
    typeof password !== "string" ||
    !(await verifyLinkPassword(password, link.passwordHash))
  )
    return NextResponse.redirect(
      new URL(`/unlock/${id}?error=1`, request.url),
      303,
    );
  ...
  response.cookies.set(linkAccessCookieName(id), createLinkAccessToken(id), {
    httpOnly: true,
    sameSite: "lax",
